...
import { joinAll } from "./join-all.ts";
...
Deno.test("joinAll should send the correct wire command for a single chunk", async () => {
  const { client, data, clientError, end } = await fakeConnection();
  clientError.catch(ignoreErrors);

  await switchToImmediateTimeout(
    () => joinAll(client, ["pajlada", "randers", "nymn_hs", "forsen"]).catch(ignoreErrors));
...
Deno.test("joinAll should send the correct wire command for a multiple chunks", async () => {
  const { client, clientError, emitAndEnd, emit, data } = await fakeConnection();
  ...
  assertStrictEquals(results["pajlada"], undefined);
  ...
  assertTrue(client.wantedChannels.has("nymn_hs"));
  assertFalse(client.joinedChannels.has("nymn_hs"));

  assertTrue(client.wantedChannels.has("pajlada"));
  assertTrue(client.joinedChannels.has("pajlada"));
