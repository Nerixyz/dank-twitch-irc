...
import { fakeClient } from "../helpers.test.ts";
import {
  AlternateMessageModifier,
  invisibleSuffix,
} from "./alternate-message-modifier.ts";

Deno.test("AlternateMessageModifier should have the correct escape for the invisible suffix",
  () => {
    // 1 (space) + 2 (invisible character)
    assertStrictEquals(invisibleSuffix.length, 3);
    assertStrictEquals([...invisibleSuffix].length, 2);
  });

Deno.test("AlternateMessageModifier should append invisible character if last message is equal",
  async () => {
...
Deno.test(
  "AlternateMessageModifier should not append invisible character if fast spam is enabled (mod, VIP, etc.)",
  async () => {
    const { client, emitAndEnd } = fakeClient();
    client.configuration.username = "randers";
    const messageModifier = new AlternateMessageModifier(client);
...
Deno.test(
  "AlternateMessageModifier should append invisible character through the say() function (case where we are joined to channel)",
  async () => {
    ...
    assertEquals(transports[1].data, [
...
Deno.test(
  "AlternateMessageModifier should append invisible character through the say() function (case where we are not joined to channel)",
  async () => {
    const { client, end, transports } = fakeClient();
    client.configuration.username = "randers";
    ...
    assertEquals(transports[0].data, [
      "PRIVMSG #forsen :Kappa Keepo PogChamp\r\n",
    ]);
...
Deno.test(
  "AlternateMessageModifier should append invisible character through the me() function (case where we are joined to channel)",
  async () => {
    const { client, end, transports } = fakeClient();
    client.configuration.username = "randers";
    client.connections[0].joinedChannels.add("forsen");
...
Deno.test(
  "AlternateMessageModifier should append invisible character through the me() function (case where we are not joined to channel)",
  async () => {
    const { client, end, transports } = fakeClient();
    client.configuration.username = "randers";
